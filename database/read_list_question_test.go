...
func TestReadListQuestion(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			q, err := tc.db.ReadListQuestion(fixtures.UserId, fixtures.ListId, fixtures.QuestionId)
			if (err != nil) != tc.wantErr {
				t.Errorf("ReadListQuestion() wantedErr = %v; got = %v", tc.wantErr, err)
			}
			if !tc.wantErr && !reflect.DeepEqual(q, fixtures.ValidQuestion) {
				t.Errorf("ReadListQuestion() wanted = %v; got = %v", fixtures.ValidQuestion, q)
			}
		})
	}
}
